...
if [ "$IS_TEST_MODE" = true ]; then
    ...
    if systemctl is-active --quiet shipyard-server; then
        print_info "Stopping existing shipyard-server service..."
        systemctl stop shipyard-server
    fi
...
if [ ! -f "$ENV_FILE" ]; then
    ENV_EXAMPLE_URL="https://raw.githubusercontent.com/youfun/shipyard/main/.env_example"
    print_info "Downloading default configuration from GitHub..."
    
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL -o "$ENV_FILE" "$ENV_EXAMPLE_URL"
    elif command -v wget >/dev/null 2>&1; then
        wget -q -O "$ENV_FILE" "$ENV_EXAMPLE_URL"
    else
        print_error "curl or wget is required to download configuration file"
        exit 1
    fi
    ...
    if [ "$IS_TEST_MODE" = false ]; then
        # Configure DB_PATH for standard installation
        if grep -q "^# DB_PATH=" "$ENV_FILE"; then
             sed -i 's|^# DB_PATH=.*|DB_PATH=/var/lib/shipyard/shipyard.db|' "$ENV_FILE"
        fi
...
Documentation=https://github.com/youfun/shipyard
...
# Start command
ExecStart=${INSTALL_DIR}/${INSTALL_NAME} --port \${SERVER_PORT:-15678}
...
echo ""
echo -e "${GREEN}Access Web UI: http://localhost:15678${NC}"
echo ""
